...
static int fill_grant_buffer(struct p9front_info *info)
{
...
static void p9_free(struct p9front_info *info, int suspend)
{
	struct grant *n;
	int i, j, segs;
...
static irqreturn_t p9_interrupt(int irq, void *dev_id)
{
	...
	RING_IDX i, rp;
	unsigned long flags;
	struct p9front_info *info = (struct p9front_info *)dev_id;
	int error;
...
static void
p9front_closing(struct p9front_info *info)
{
 ...
 */
static void p9front_connect(struct p9front_info *info)
{
	struct grant *gnt_list_entry = NULL;
	err = fill_grant_buffer(info, 1);
	if (err) {
	 ...
	 */
	data_xfer[0] = 5;
        gnt_list_entry = get_grant(&gref_head, page_to_pfn(data_xfer), info);
	spin_unlock_irq(&info->io_lock);
	return 0;
}
...
{
	struct p9front_info *info = dev_get_drvdata(&dev->dev);
...
{
	struct p9front_info *info = dev_get_drvdata(&xbdev->dev);
